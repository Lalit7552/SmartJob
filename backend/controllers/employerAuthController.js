...
async function revokeSessions({ employerId, kind }) {
  await EmployerSession.updateMany(
    { employerId, kind, revokedAt: { $exists: false } },
    { $set: { revokedAt: new Date() } }
  );
}

async function createSession({ employerId, kind, currentStep, ttlSec, req }) {
  const expiresAt = new Date(Date.now() + ttlSec * 1000);
  const session = await EmployerSession.create({
    employerId,
    kind,
    currentStep,
    expiresAt,
...
async function verifyOtp(req, res, next) {
  try {
    ...
    const isComplete = employer.status === "active" || employer.onboardingStep === "done";
    if (isComplete) {
      await revokeSessions({ employerId: employer._id, kind: "auth" });
      ...
      return res.json({
        ok: true,
        data: {
          user: { id: employer._id.toString(), isNew: false, phone: employer.phone },
    ...
    await revokeSessions({ employerId: employer._id, kind: "onboarding" });
    const { token, session } = await createSession({
      employerId: employer._id,
      kind: "onboarding",
      currentStep: employer.onboardingStep,
    ...
    return res.json({
      ok: true,
      data: {
        user: { id: employer._id.toString(), isNew, phone: employer.phone },
        session: { kind: "onboarding", token, expiresAt: session.expiresAt },
        next: nextEmployerResponse({ isNew, onboardingStep: employer.onboardingStep, kind: "onboarding" }),
      },
    });
  } catch (err) {
    return next(err);
