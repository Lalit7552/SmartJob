...
import { RefreshCw, Trash2, RotateCcw, Briefcase } from "lucide-react";
...
export default function ActiveJobs() {
  ...
  useEffect(() => {
    let data = [...jobs];
    data = data.filter((j) => j.type !== "One Day Job");
    switch (filterStatus) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8">
        <div>
          ...
          <p className="text-[11px] font-semibold text-slate-500 tracking-[1.5px] uppercase mt-1">
            Jobs created from employer post job page
          </p>
        </div>
        <button
          onClick={load}
                  ...
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mt-0.5">
                        {job.category || "General"} · {job.type || "Full Time"}
                      </p>
