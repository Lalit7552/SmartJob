...
import { requestWorkerOtp, verifyWorkerOtp } from "../../api/workerApi";
...
export default function WorkerSignup() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setMessage("");
      setLoading(true);
      await requestWorkerOtp({ phone, countryCode: "+91" });
      setStep("otp");
  ...
  const handleVerify = async () => {
    ...
    try {
      setMessage("");
      setLoading(true);
      const res = await verifyWorkerOtp({ phone, otp: otp.join(""), countryCode: "+91" });
      ...
      <div className="bg-black/20 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center shadow-lg shadow-black/20">
            <span className="text-[#004D36] font-black text-xs italic">NW</span>
          </div>
          <span className="font-black text-white uppercase tracking-tighter">
            Naukari<span className="text-white/40 italic font-medium">Wala</span>
          </span>
        </div>
