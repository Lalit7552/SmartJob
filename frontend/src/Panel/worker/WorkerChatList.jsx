...
export default function WorkerChatList() {
  ...
  const filtered = useMemo(() => {
    ...
    return threads.filter((t) =>
      String(t?.contactName || "Employer").toLowerCase().includes(q)
    );
  }, [threads, query]);
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white/60 p-16 text-center">
            ...
            <p className="text-slate-400 text-xs font-medium">
              Start a chat from any job card.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((t) => {
              const employerId = t.employerId;
              const name = t.contactName || "Employer";
              ...
              return (
                <button
                  key={employerId}
                  onClick={() => navigate(`/worker/chat/${employerId}`)}
