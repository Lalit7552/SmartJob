...
export default function WorkerChatPage() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (alive) setWorker(res?.data?.user || null);
      } catch {
    ...
    const loadThreads = async () => {
      try {
        ...
        const found = (res?.data?.threads || []).find(
          (t) => String(t.employerId) === String(employerId)
        );
        if (alive) setEmployerName(found?.contactName || "Employer");
      } catch {
        if (alive) setEmployerName("Employer");
      }
    };
    loadThreads();
    return () => { alive = false; };
  ...
  useEffect(() => {
    ...
    socket.on("chat:new", (msg) => {
      if (String(msg.employerId) !== String(employerId)) return;
      setMessages((prev) => {
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      try {
        setLoading(true);
        setMessages([]);
        const res = await fetchChatMessages({ workerId, employerId, role: "worker" });
  ...
  const sendMessage = () => {
    ...
    if (!trimmed || !workerId || !employerId) return;
  ...
  const initials = useMemo(
    () => (employerName || "E").split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase(),
    [employerName]
  );
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 flex items-center gap-3 border-b border-slate-200/70 bg-white/80 backdrop-blur">
        ...
        <div>
          <p className="text-sm font-extrabold text-slate-900">{employerName}</p>
